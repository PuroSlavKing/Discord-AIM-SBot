...
async def on_message(self, message):
    ...
    if message.author.id == self.bot.user.id and not message.content.startswith(
            config['GENERAL']['prefix']) and not 'Selfbot' in message.content:
        global text_mode
    ...
    if reactionbot['enabled'] and message.guild.id == int(reactionbot['server_id']) or reactionbot['enabled'] and \
        ...
        try:
            ...
            pass
    if troll['mode'] == 3 and message.author.id == troll['user_id']:
...
def to_color(text):
    ...
    if text_mode == 'rainbow':
        ...
        colors_bad = [Fore.CYAN, Fore.BLUE]
    elif text_mode == 'white':
        colors_bad = [Fore.WHITE]
    else:
        ...
        while True:
            for webhook in await ctx.channel.webhooks():
                try:
                    ...
                    continue
                return
            webhook = await ctx.channel.create_webhook(name='AIM')
